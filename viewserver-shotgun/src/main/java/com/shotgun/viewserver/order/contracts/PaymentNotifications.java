...
public interface PaymentNotifications extends OrderNotificationContract {
    ...
    default void notifyPaymentStageStarted(String orderId, String customerId, String description) {
        User user = (User) ControllerContext.get("user");
        sendMessage(orderId, customerId, "Shotgun payment stage started", String.format("%s has  just marked part of your job \"%s\" started", user.getFirstName() + " " + user.getLastName(), description),true);
    }

    default void notifyPaymentStageComplete(String orderId, String customerId, String description) {
        User user = (User) ControllerContext.get("user");
        sendMessage(orderId, customerId, "Shotgun Job Stage Complete", String.format("%s has  just marked part of your job \"%s\" complete. Please log in to pay", user.getFirstName() + " " + user.getLastName(), description),true);
    }

    default void notifyPaymentStagePaid(String orderId, String partnerId, String description) {
        User user = (User) ControllerContext.get("user");
        sendMessage(orderId, partnerId, String.format("Shotgun Job Stage Paid"), String.format("%s has  just paid for stage \"%s\" complete.", user.getFirstName() + " " + user.getLastName(), description),false);
    }
