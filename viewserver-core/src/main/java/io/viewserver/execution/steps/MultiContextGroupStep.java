 ...
 */
public class MultiContextGroupStep implements IExecutionPlanStep<MultiContextExecutionPlanContext> {
    @Override
    public void execute(MultiContextExecutionPlanContext executionPlanContext) {
        GroupByNode groupByNode = new GroupByNode("comparisonGroupBy")
                .withGroupByColumns("sourceId")
                .withRegexSummary("$1", "avg", "(bucket.+)")
                .withConnection(executionPlanContext.getInputOperator());
        executionPlanContext.addNodes(groupByNode);
        executionPlanContext.setInput(groupByNode.getName());
    }
}
