...
public class EqualsString implements IExpressionBool {
        private final IExpressionString lhs;
        private final IExpressionString rhs;
    private boolean isLiteral;
    private boolean isEqual;

        public EqualsString(IExpressionString lhs, IExpressionString rhs) {
            this.lhs = lhs;
            this.rhs = rhs;

            if (lhs instanceof LiteralString && rhs instanceof LiteralString) {
                isLiteral = true;
                isEqual = Objects.equal(lhs.getString(0), rhs.getString(0));
            }
        }

        @Override
        public boolean getBool(int row) {
            return isLiteral ? isEqual : Objects.equal(lhs.getString(row), rhs.getString(row));
        }
