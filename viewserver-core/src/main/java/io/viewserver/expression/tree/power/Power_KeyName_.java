...
public class Power_KeyName_ implements IExpression_KeyName_ {
    private final IExpression_KeyName_ lhs;
    private final IExpression_KeyName_ rhs;

    public Power_KeyName_(IExpression_KeyName_ lhs, IExpression_KeyName_ rhs) {
        this.lhs = lhs;
        this.rhs = rhs;
    }
    ...
    public _KeyType_ get_KeyName_(int row) {
        return (_KeyType_) Math.pow((double)lhs.get_KeyName_(row), (double)rhs.get_KeyName_(row));
    }
