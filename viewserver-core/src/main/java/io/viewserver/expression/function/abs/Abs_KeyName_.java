...
public class Abs_KeyName_ implements IExpression_KeyName_ {
    private final IExpression_KeyName_ value;

    public Abs_KeyName_(IExpression_KeyName_ value) {
        this.value = value;
    }

    @Override
    public _KeyType_ get_KeyName_(int row) {
        return (_KeyType_)Math.abs(value.get_KeyName_(row));
    }
