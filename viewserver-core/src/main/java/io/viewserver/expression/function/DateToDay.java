 ...
 */
public class DateToDay implements IUserDefinedFunction, IHookingFunction, IExpressionInt {
    private static final Logger log = LoggerFactory.getLogger(DateToDay.class);
    ...
    public void setParameters(IExpression... parameters) {
        if (parameters.length < 1 || !(parameters[0] instanceof IExpressionLong)
                ...
                || parameters.length > 2) {
            throw new IllegalArgumentException("Syntax: day(<date (long)>[, <rollover (boolean)>])");
        }
    ...
    public int getInt(int row) {
        return Days.daysBetween(new DateTime(dateExpression.getLong(row), DateTimeZone.UTC), DateTime.now(DateTimeZone.UTC)).getDays();
    }
