...
public class ConsistentHash {
     ...
     */
    public void addBin(int bin) {
        for (int i = 0; i < numberOfReplicas; i++) {
            // The string addition forces each replica to have different hash
            circle.put(hashFunction.hashString(Long.toString(bin) + i, charset).padToLong(), bin);
        }
    }
    ...
    public void removeBin(int bin) {
        for (int i = 0; i < numberOfReplicas; i++) {
            ...
            circle.remove(hashFunction.hashString(Long.toString(bin) + i, charset).padToLong());
        }
    }
     ...
     * subsequent bin.
     */
    public int getBinFor(long key) {
        if (circle.isEmpty()) {
            return Integer.MIN_VALUE;
        }
        long hash = hashFunction.hashLong(key).padToLong();
        Integer bin = circle.get(hash);
...
//     * duplicates.
//     */
//    public List<Integer> getNBinsFor(long key, int n) {
//        if (circle.isEmpty()) {
