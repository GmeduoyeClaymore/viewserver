 ...
 */
public class FifoBuffer_KeyName_ {
    private _KeyType_[] elements;
    private int start = 0;
    private int end = 0;
    private int maxElements;
    private boolean full;

    public FifoBuffer_KeyName_(int size) {
    ...
    private void resize() {
        ...
        _KeyType_[] oldElements = elements;
        elements = new _KeyType_[newSize];
        System.arraycopy(oldElements, start, elements, 0, maxElements - start);
        if (start > 0) {
            System.arraycopy(oldElements, 0, elements, maxElements - start, start);
        }
        end = maxElements;
        start = 0;
        maxElements = newSize;
