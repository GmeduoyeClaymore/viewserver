 ...
 */
public class ColumnMultikeyQuicksort {
     ...
     */
    private ColumnMultikeyQuicksort() {
    }
     ...
     */
    private static void vecswap(int[] a, int i, int j, int n) {
        while (n-- > 0) {
            int t = a[i];
            a[i] = a[j];
            a[j] = t;
    ...
    public static void sort(int[] strings, ColumnHolderString columnHolder) {
        if (strings != null && strings.length > 1) {
            ssort(strings, columnHolder, 0, strings.length, 0);
        }
    }
    ...
    public static void sort(int[] strings, ColumnHolderString columnHolder, int low, int high, int depth) {
        if (strings != null && strings.length > 1 && low >= 0 && low < high && depth >= 0) {
            ssort(strings, columnHolder, low, high - low, depth);
        }
    }
     ...
     */
    private static int med3(int[] a, ColumnHolderString columnHolder, int l, int m, int h, int depth) {
        char va = charAt(columnHolder.getString(a[l]), depth);
        char vb = charAt(columnHolder.getString(a[m]), depth);
        if (va == vb) {
            return l;
        }
        char vc = charAt(columnHolder.getString(a[h]), depth);
        if (vc == va || vc == vb) {
            return h;
        }
     ...
     */
    private static void ssort(int[] a, ColumnHolderString columnHolder, int base, int n, int depth) {
        if (n < THRESHOLD) {
            ColumnInsertionsort.sort(a, columnHolder, base, base + n, depth);
            return;
        }
        int pl = base;
        int pm = base + n / 2;
        ...
        if (n > 30) {
            ...
            int d = n / 8;
            pl = med3(a, columnHolder, base, base + d, base + 2 * d, depth);
            pm = med3(a, columnHolder, base + n / 2 - d, pm, base + n / 2 + d, depth);
            pn = med3(a, columnHolder, base + n - 1 - 2 * d, base + n - 1 - d, pn, depth);
        }
        pm = med3(a, columnHolder, pl, pm, pn, depth);
        int t = a[base];
        a[base] = a[pm];
        a[pm] = t;
        int v = charAt(columnHolder.getString(a[base]), depth);
        boolean allzeros = v == 0;
        int le = base + 1, lt = le;
        int gt = base + n - 1, ge = gt;
        while (true) {
            for (; lt <= gt && (r = charAt(columnHolder.getString(a[lt]), depth) - v) <= 0; lt++) {
                if (r == 0) {
                    t = a[le];
                    a[le] = a[lt];
                    a[lt] = t;
            ...
            for (; lt <= gt && (r = charAt(columnHolder.getString(a[gt]), depth) - v) >= 0; gt--) {
                if (r == 0) {
                    t = a[gt];
                    a[gt] = a[ge];
                    a[ge] = t;
        ...
        vecswap(a, lt, pn - r, r);
        if ((r = lt - le) > 1) {
            ssort(a, columnHolder, base, r, depth);
        }
        if (!allzeros) {
            // Only descend if there was at least one string that was
            // of equal or greater length than current depth.
            ssort(a, columnHolder, base + r, le + n - ge - 1, depth + 1);
        }
        if ((r = ge - gt) > 1) {
            ssort(a, columnHolder, base + n - r, r, depth);
        }
    }
}
