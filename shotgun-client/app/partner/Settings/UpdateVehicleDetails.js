...
import {updateVehicle, getVehicleDetails} from 'partner/actions/PartnerActions';
...
class UpdateVehicleDetails extends Component {
  ...
  onUpdateVehicle = async() => {
    ...
    dispatch(updateVehicle(vehicle, () => history.push(`${parentPath}/PartnerSettingsLanding`)));
  };
...
const mapStateToProps = (state, nextOwnProps) => {
  ...
  const vehicleDetails =  getDaoState(state, ['vehicleDetails'], 'partnerDao');
  const {vehicleId} = currentVehicle;
  const vehicle = {...(vehicleDetails || currentVehicle), vehicleId};

  return {
    ...nextOwnProps,
    getVehicleDetailsBusy: isAnyOperationPending(state, [{partnerDao: 'getVehicleDetails'}]),
    busy: isAnyOperationPending(state, [{vehicleDao: 'addOrUpdateVehicle'}]),
    errors: getOperationErrors(state, [{vehicleDao: 'addOrUpdateVehicle'}, {partnerDao: 'getVehicleDetails'}]),
    vehicle
  };
};
