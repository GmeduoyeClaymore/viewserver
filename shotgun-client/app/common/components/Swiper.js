...
export class Swiper extends Component {
  ...
  initState (props, updateIndex = false) {
    ...
    console.log(`Selected new index is ${initState.index} offset is ${JSON.stringify(initState.offset[initState.dir])}`);
  ...
  updateIndex = (offset, dir, cb) => {
    ...
    if (loopJump) {
      ...
      // after render.
      if (offset[dir] === this.internals.offset[dir] || false) {
        newState.offset = { x: 0, y: 0 };
        newState.offset[dir] = offset[dir] + 1;
        this.setState(newState, () => {
          this.setState({ offset: { x: 0, y: 0 }}, cb);
          console.log('Selected Offset is ' + JSON.stringify({ x: 0, y: 0 }) + ' index is ' + newState.index );
        });
      } else {
        newState.offset = offset;
        console.log('Selected Offset is ' + JSON.stringify(offset) + ' index is ' + newState.index );
  ...
  render () {
    ...
    if (total > 1) {
      ...
      console.log(`Sorted pages are ${pages.join(',')}`);
