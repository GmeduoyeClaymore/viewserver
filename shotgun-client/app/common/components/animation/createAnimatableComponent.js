...
export default function createAnimatableComponent(WrappedComponent) {
  ...
  return class AnimatableComponent extends Component {
    ...
    animate(animation, duration, propsOverride) {
      return new Promise(resolve => {
        this.setAnimation(animation, () => {
          this.startAnimation(duration, 0, resolve, propsOverride);
        });
      });
    }
    ...
    startAnimation(duration, iteration, callback , propsOverride = {}) {
      const { animationValue, compiledAnimation } = this.state;
      let { directionFromProps, iterationCountFromProps, useNativeDriverFromProps } = this.props;
      let { direction = directionFromProps, iterationCount = iterationCountFromProps, useNativeDriver = useNativeDriverFromProps } = propsOverride;
